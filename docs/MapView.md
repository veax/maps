...
| localizeLabels | \| { <br/> /** locale code like `es` or `current` for the device's current locale \*/ locale: string; <br/><br/>/** layer id to localize. If not specified, all layers will be localized */ <br/> layerIds?: string[];<br/>}<br/>\| true | `none` | `false` | [`mapbox` (v10) implementation only]<br/>Set map's label locale, e.g. { "locale": "es" } will localize labels to Spanish, { "locale": "current" } will localize labels to system locale. |
